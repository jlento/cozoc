...
int read2D (
    ...
    count[2] = xm;
    ERR (nc_inq_varid (ncid, varname, &id) );
    ERR (nc_get_vara_double (ncid, id, start, count, &a[start[1]][start[2]]) );
    DMDAVecRestoreArray (da, v, &a);
    return (0); }
...
int file_read_array_double (
    ...
    info ("Reading variable %s\n", varname);
    ERR (nc_inq_varid (ncid, varname, &id) );
    ERR (nc_get_vara_double (ncid, id, start, count, a_double) );
    return (0); }
...
int write3Ddump (const char *varname, size_t mx, size_t my, size_t mz, Vec v) {
    ...
    ERR (
        nc_create_par (
            fname, NC_MPIIO | NC_NETCDF4, PETSC_COMM_WORLD, MPI_INFO_NULL,
            &ncid) );

    for (int i = 0; i < ndims; i++)
        ERR (nc_def_dim (ncid, dnames[i], ds[i], &dimids[i]) );

    ERR (nc_def_var (ncid, varname, NC_FLOAT, ndims, dimids, &varid) );
    ERR (nc_enddef (ncid) );
